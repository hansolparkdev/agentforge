...
```
# 첫 번째 기획
/forge 틱택토                      → docs/plans/tic-tac-toe/plan.md
/slice tic-tac-toe                 → docs/plans/tic-tac-toe/features.md
/build tic-tac-toe F1
/build tic-tac-toe F2

# 같은 프로젝트에 기능 추가
/forge 로그인                      → docs/plans/login/plan.md
/slice login                       → docs/plans/login/features.md
/build login F1
```
...
**중단 재개:**
`docs/project/status.md`에 진행 상황이 저장된다. `/build {slug} F{n}` 재실행 시 중단 위치부터 이어서 진행할지 선택할 수 있다.
