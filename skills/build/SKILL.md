---
name: build
description: TDD 기반 개발을 실행한다. "/build F{n}" 또는 기획서 없이 "/build {작업설명}"으로 호출 가능. /slice로 features.md가 생성된 경우 Feature 단위로, 없는 경우 자유형 태스크로 동작한다.
---
...
/build F2
```
`docs/plans/features.md`의 Feature 정의를 기준으로 동작한다.
...
/build 결제 페이지 UI
```
사용자가 입력한 작업 설명을 Feature 정의로 사용한다.
...
**A. 기획서 기반인 경우:**
- `docs/plans/features.md` 존재 확인 (읽지 않는다)
- 없으면: "Feature 목록이 없습니다. `/slice`를 먼저 실행하세요."
...
- `진행 중: {작업} — {에이전트명}` 항목이 있으면 사용자에게 알린다:
  ```
  이전 빌드가 중단된 기록이 있습니다.
  중단 위치: {작업} — {에이전트명}
  이어서 진행하시겠습니까? (y/n)
...
```
feature: {F{n} | 작업 설명}
기획서: docs/plans/plan.md (있는 경우)
features: docs/plans/features.md (있는 경우)
```
...
다음 중 선택해주세요:
1. 문제를 해결하고 "/build F{n}" 재실행
2. docs/plans/ambiguous.md 확인 후 스펙 보완 및 재실행
3. 이 Feature를 건너뛰고 "/build F{n+1}" 실행
```
