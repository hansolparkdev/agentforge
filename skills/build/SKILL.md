---
name: build
description: TDD 기반 개발을 실행한다. "/build {slug} F{n}" 또는 기획서 없이 "/build {작업설명}"으로 호출 가능. /slice로 features.md가 생성된 경우 Feature 단위로, 없는 경우 자유형 태스크로 동작한다.
---
...
/build 결제 페이지 UI
```
사용자가 입력한 작업 설명을 Feature 정의로 사용한다. slug는 작업 설명에서 자동 생성한다.
...
- slug = 작업 설명에서 생성한 kebab-case (예: `login-api`, `payment-ui`)
